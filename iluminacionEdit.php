	        ...
	        <div class="columna col-4">
	        	<form method="POST" action="">
					<div class="row form-group">
						<h3>EDITAR ILUMINACIÓN</h3>
						<hr class="bg-info">
						<p class="pb-0 mb-0">La Unidad de Medida esta en Lumens</p>
						...
						<input type="submit" name="editarIlum" value="Editar" class="btn btn-success">
					</div>
				</form>
	        </div>
	        <?php 
	        	$conn = mysqli_connect("localhost", "root", "", "sistema_riego");
				if (isset($_POST['editarIlum'])) {
					$editarIlum = $_POST['ilumN'];
					$editarId = 1;

					$editar_sql = "UPDATE iluminacionbase SET ilum='$editarIlum' WHERE id= '$editarId'";

					$ejecutar = mysqli_query($conn, $editar_sql);

					if ($ejecutar) {
						echo "<script>alert('Iluminación Editada')</script>";
